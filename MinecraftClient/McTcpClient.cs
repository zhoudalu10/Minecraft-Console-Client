...
namespace MinecraftClient
{
    ...
    public class McTcpClient : IMinecraftComHandler
    {
        ...
        public bool ClickWindowSlot(int windowId, int slotId, int button, int mode)
        {
            Item item = null;
            ...
            if (mode == 2)
            {
                if (inventories[windowId].Items.ContainsKey(slotId) && inventories[windowId].Items.ContainsKey(button + 36))
                {
                    Item tmp = inventories[windowId].Items[slotId];
                    inventories[windowId].Items[slotId] = inventories[windowId].Items[button + 36];
                    inventories[windowId].Items[button + 36] = tmp;
                }
                else if (inventories[windowId].Items.ContainsKey(slotId))
                {
                    inventories[windowId].Items[button + 36] = inventories[windowId].Items[slotId];
                    inventories[windowId].Items.Remove(slotId);
                }
                else if (inventories[windowId].Items.ContainsKey(button + 36))
                {
                    inventories[windowId].Items[slotId] = inventories[windowId].Items[button + 36];
                    inventories[windowId].Items.Remove(button + 36);
                }
                item = new Item(-1, 0, new Dictionary<string, object>());
            }
            else
            {
            ...
            return handler.SendClickWindow(windowId, slotId, button, mode, item);
        }
        ...
        public bool PlaceBlock(int hand, Location location)
        {
            //WORK IN PROGRESS. MAY NOT WORK YET
            if (Settings.DebugMessages)
                ConsoleIO.WriteLogLine(location.ToString());
            return handler.SendPlayerBlockPlacement(hand, location, 1, 0.5f, 0.5f, 0.5f, false);
        }
        ...
        public bool DiggingBlock(int status, Location location)
        {
            Material block = GetWorld().GetBlock(location).Type;
            ConsoleIO.WriteLogLine("Block name : " + block.ToString());
            if (block.IsLiquid())
            {
                ConsoleIO.WriteLogLine("Can't dig liquid");
                return false;
            }
            ...
            switch (block)
            {
                ...
                case Material.NetherBricks:
                    digTime = 400;
                    digTime = (int)((float)digTime / 3.25);
                    break;
            ...
            if (Settings.DebugMessages)
                ConsoleIO.WriteLogLine(location.ToString());
            return handler.SendPlayerDigging(status, location, 1, digTime);
        }
    }
}
